        ...
        {view === "conversation" && (!sessionStarted ? (
          ...
          <section className="visionConversation">
            ...
            <div className="visionThread">
              ...
              {activeRule && sourceMode === "live" && selectedCamera && <MonitoringFeed agent={agent} alerts={currentAlerts} analysisLabel={activeRule.execution_plan?.strategy === "specialized_pose" ? "YOLO pose" : activeRule.execution_plan?.strategy === "semantic_window" ? "Visual AI" : "YOLO tracking"} busy={busy || working} camera={selectedCamera} detections={detections} events={currentEvents} nativePreviewEnabled={nativePreviewEnabled} onPreviewAvailabilityChange={setPreviewReady} onStop={stopCameraAndRecording} operating={operating} previewReady={previewReady} recordings={recordings} running={Boolean(running)} stream={stream} />}
            </div>
          </section>
        ))}
